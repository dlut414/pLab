...
function setupShader_2D(a, c) {
  ...
  return (
    ...
    gl.detachShader(h, d),
    gl.detachShader(h, f),
    gl.getProgramParameter(h, gl.LINK_STATUS)
      ? h
      : void alert("Shader program did not link successfully!")
  );
}
function setupCanvas_2D() {
  return ((gl = canvas.getContext("experimental-webgl")), !gl)
    ? null
...
function setupData_2D(a) {
  ...
  for (var d = c[0].split(/(?:\s)+/).length, f = 0; f < d; f++) {
    $("#selected_scalar").append(
      "<option value=" + (f + ">") + (f + "</option>")
    ),
      data.push(new Float32Array(c.length));
  }
  for (var f = 0; f < c.length; f++) {
    ...
    (pos_avg[0] += l),
      (pos_avg[1] += m),
      (pos_max[0] = pos_max[0] > l ? pos_max[0] : l),
      (pos_max[1] = pos_max[1] > m ? pos_max[1] : m),
      (pos_min[0] = pos_min[0] < l ? pos_min[0] : l),
      (pos_min[1] = pos_min[1] < m ? pos_min[1] : m);
  }
  (pos_avg[0] /= c.length),
    (pos_avg[1] /= c.length),
...
function point_2D() {
  return (
    ...
    (this.draw = function() {
      ...
      var h = gl.getAttribLocation(this.program, "type");
      ...
      var k = gl.getAttribLocation(this.program, "x");
      gl.enableVertexAttribArray(k),
        gl.vertexAttribPointer(k, 1, gl.FLOAT, !1, 0, 0),
        gl.bindBuffer(gl.ARRAY_BUFFER, d),
        gl.bufferData(gl.ARRAY_BUFFER, data[2], gl.STATIC_DRAW);
      var l = gl.getAttribLocation(this.program, "y");
      gl.enableVertexAttribArray(l),
        gl.vertexAttribPointer(l, 1, gl.FLOAT, !1, 0, 0),
        gl.bindBuffer(gl.ARRAY_BUFFER, f),
        gl.bufferData(gl.ARRAY_BUFFER, data[selected_col], gl.STATIC_DRAW);
      var m = gl.getAttribLocation(this.program, "s");
      gl.enableVertexAttribArray(m),
        gl.vertexAttribPointer(m, 1, gl.FLOAT, !1, 0, 0),
...
function colorPick() {
  return (
    ...
    (this.pick = function(a, c) {
      if (!gl) return void alert("WebGL not available!");
      gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT),
        gl.useProgram(this.program);
      var d = gl.createBuffer(),
        f = gl.createBuffer(),
        h = gl.createBuffer();
      gl.uniform1f(gl.getUniformLocation(this.program, "range_max"), range_max),
        gl.uniform1f(
      ...
      for (var m = 0; m < data[0].length; m++) {
        var n = ((255 & m) >> 0) / 255,
          o = ((65280 & m) >> 8) / 255,
          q = ((16711680 & m) >> 16) / 255;
        gl.uniform3fv(gl.getUniformLocation(this.program, "color"), [n, o, q]),
          gl.drawArrays(gl.POINTS, m, 1);
      }
      gl.disableVertexAttribArray(k),
        gl.disableVertexAttribArray(l),
        gl.flush(),
      ...
      return (
        gl.readPixels(a, c, 1, 1, gl.RGBA, gl.UNSIGNED_BYTE, s),
        s[0] + (s[1] << 8) + (s[2] << 16)
      );
    }),
    this
...
$("#canvas").mousedown(function(a) {
  (mouse_down = !0), (this.pre = coordInCanvas(a));
}),
  $("#canvas").mouseup(function(a) {
    if (((mouse_down = !1), mode == dict.POINT_MODE)) {
      var c = colorPicker.pick(
          a.pageX - a.target.offsetLeft,
          canvas_size[1] - (a.pageY - a.target.offsetTop)
        ),
        d = $("#attribPanel");
      try {
        d.html(
  ...
  $("#canvas").mousemove(function(a) {
    if (mouse_down) {
      var c = coordInCanvas(a);
      (pan[0] += c[0] - this.pre[0]),
        (pan[1] += c[1] - this.pre[1]),
        (this.pre = c),
        render_2D();
    }
  }),
  $("#canvas").on("wheel", function(a) {
    a.stopPropagation(),
      a.preventDefault(),
      0 > a.originalEvent.deltaY
        ? (scale *= 1.1)
  ...
  $("#mode").change(function() {
    var a = $("#form-point"),
      c = $("#form-streamline");
    $("#radio-pt").is(":checked")
      ? (a.show("fast"), c.hide("fast"), (mode = dict.POINT_MODE), render_2D())
      : $("#radio-sl").is(":checked") &&
        (c.show("fast"),
        a.hide("fast"),
