...
function setupData_2D(result) {
	...
	for (var i = 0; i < lines.length; i++) {
		...
		var x = Number(line[1]),
		    y = Number(line[2]);
		var vx = Number(line[3]),
		    vy = Number(line[4]);
		data[0][i] = type;
		data[1][i] = x, data[2][i] = y;
		data[3][i] = vx, data[4][i] = vy;
		for (var j = 5; j < n_cols; j++) {
			data[j][i] = Number(line[j]);
...
function point_2D() {
	...
	this.draw = function () {
		...
		var vbo_x = gl.createBuffer(),
		    vbo_y = gl.createBuffer(),
		    vbo_s = gl.createBuffer();
...
function streamline_2D() {
	...
	this.draw = function () {
		...
		gl.useProgram(this.program);
		var vbo_linex = gl.createBuffer(),
		    vbo_liney = gl.createBuffer();
